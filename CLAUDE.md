...
- **Category hierarchy**: `Category` has a self-referential, optional `parent_id`, capped at exactly 2 levels (a subcategory's parent must itself be top-level — enforced in `main.py`, not at the DB level). A subcategory's `type` must match its parent's, and a category with existing subcategories can't have its own `type` or `parent_id` changed. Both parent and child categories remain directly assignable to a transaction; subcategories don't inherit the parent's split-weight tier or roll up into it for charts.
...
- **Backend layout**: `main.py` is app wiring only — the `FastAPI` app, CORS, the four exception handlers, and `include_router` calls. Route handlers live in `routers/` (`users`, `accounts`, `categories`, `transactions`, `splits`, `data_io`, `dashboard`), one module per resource. Supporting them: `serializers.py` (every ORM→Pydantic `*Out` mapping, plus `TRANSACTION_LOAD_OPTIONS`/`transaction_query()` — use these rather than hand-rolling a join, or you reintroduce an N+1 on splits), `rules.py` (domain rules raising `RuleViolation`, translated to 422 by the handler in `main.py`), `models.py` (SQLAlchemy ORM), `schemas.py` (Pydantic I/O schemas), `database.py` (engine/session, `sync_schema()`), `seed.py` (destructive seeder), `alembic/` (migration environment + `versions/`, see "Database migrations" above), plus `accounting_month.py`, `audit.py`, `backup.py`, `charts.py`, `filtering.py` (advanced-filter clauses + `visible_transaction_filter()`), `import_csv.py`, `split_engine.py`.
- **Where new backend code goes**: a new response field → `serializers.py` (one edit, not one per route); a new cross-field or DB-dependent invariant → `rules.py`, raising `RuleViolation`, unit-tested directly in `tests/test_rules.py` with no TestClient; a new endpoint → the matching `routers/` module. Routes should stay thin: validate via `rules`, mutate, serialize via `serializers`. Note `PUT /api/transactions/bulk-update` must stay registered before `/{transaction_id}` in `routers/transactions.py` — Starlette matches in registration order.
