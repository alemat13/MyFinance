...
| `raw_source` | String(20) | Nullable. Which feed wrote the `raw_*` columns below: `enable_banking` for a row pulled in by [Bank sync](#bank_connections), `linxo_export` for migrated history backfilled from the Linxo GDPR export. `NULL` for everything else. It exists because the two feeds share these columns but not their vocabularies — `raw_transaction_code` is an ISO 20022 code from a bank and a Linxo transaction type from the export, and nothing tells them apart from the value alone |
